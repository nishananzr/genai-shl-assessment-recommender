...
def calculate_recall_at_k(predicted_urls, relevant_urls, k):
    """Calculates Recall@K."""
    if not relevant_urls:
        return 0.0 
    
    predicted_at_k = predicted_urls[:k]
    num_relevant_in_predicted_at_k = len(set(predicted_at_k) & set(relevant_urls))
    recall = num_relevant_in_predicted_at_k / len(relevant_urls)
    return recall

def calculate_ap_at_k(predicted_urls, relevant_urls, k):
    ...
    ap = 0.0
    hits = 0
    relevant_urls_set = set(relevant_urls) 

    for i, p_url in enumerate(predicted_urls[:k]):
        if p_url in relevant_urls_set:
            hits += 1
    ...
    return ap / min(k, len(relevant_urls)) 


def main():
    print("--- Initializing Recommender for Evaluation ---")
    recommender.load_data()
    if not recommender.assessments_data:
        print("ERROR: No assessment data loaded from recommender. Aborting evaluation.")
        return
    recommender.initialize_model_and_embeddings()
    if not (recommender.model is not None and recommender.assessment_embeddings is not None):
        print("ERROR: Recommender model or embeddings failed to initialize. Aborting evaluation.")
        return
    ...
    print(f"\n--- Running Evaluation (K={K_FOR_METRICS}) ---")
    for item in GROUND_TRUTH_DATA:
        query = item["query"]
        true_relevant_urls = item["relevant_assessment_urls"]

        print(f"\nQuery: \"{query[:100]}...\"") 
        
        system_recommendations_full = recommender.recommend_assessments(query, top_n=10) 
        
        predicted_assessment_urls = [rec['url'] for rec in system_recommendations_full]

        recall_k = calculate_recall_at_k(predicted_assessment_urls, true_relevant_urls, K_FOR_METRICS)
        ap_k = calculate_ap_at_k(predicted_assessment_urls, true_relevant_urls, K_FOR_METRICS)

        all_recalls_at_k.append(recall_k)
        all_aps_at_k.append(ap_k)

        print(f"  Recommended URLs (Top {len(predicted_assessment_urls)}): {predicted_assessment_urls}")
        print(f"  Ground Truth URLs: {true_relevant_urls}")
        print(f"  Recall@{K_FOR_METRICS}: {recall_k:.4f}")
        print(f"  AP@{K_FOR_METRICS}: {ap_k:.4f}")

    if not all_recalls_at_k or not all_aps_at_k:
        print("\nNo evaluation results to average. Did GROUND_TRUTH_DATA have any entries?")
        return

    mean_recall_at_k = np.mean(all_recalls_at_k)
    mean_ap_at_k = np.mean(all_aps_at_k) 

    print("\n--- Evaluation Summary ---")
    print(f"Mean Recall@{K_FOR_METRICS} (MR@{K_FOR_METRICS}): {mean_recall_at_k:.4f}")
    print(f"Mean Average Precision@{K_FOR_METRICS} (MAP@{K_FOR_METRICS}): {mean_ap_at_k:.4f}")
